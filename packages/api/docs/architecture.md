...
│   └── features/           # 本体に統合されたサブ機能 (turnstile, imageUpload)
└── docs/                   # ドキュメント
```

> 旧 `plugins/`（twoCh, datImport, turnstileApiToken, imageUploader）はモノレポ統合時に整理した。
> twoCh・datImport は廃止。turnstileApiToken・imageUploader は別Workerとして運用する意義が薄かった
> （本体とKV/D1を共有するだけで、コードレベルの結合はゼロだった）ため、`src/features/` として本体に統合した。
...
board/thread/post は `acl: ResourceAcl` を1つ持つ (D1上は `acl` 列にJSON文字列で格納)。
...
interface PermissionGrant {
  ...
  actions: AclAction[]    // 階層(admin>member>user>anon)を前提としない自由な組み合わせ
}

interface ResourceAcl {
  ownerUserId: string | null       // 作成者は常にフルアクセス (旧 $CREATOR 相当)
  grants: PermissionGrant[]        // 任意のロール/ユーザーへの明示的グラント (旧 administrators/members 相当)
  authenticatedActions: AclAction[] // grants に該当しないログイン済みユーザーへのフォールバック
  anonymousActions: AclAction[]     // 未ログインユーザーへのフォールバック
}
```

`can(acl, ctx, action)` が唯一の判定関数。`isSysAdmin` が true の場合は常に許可 (全権限バイパス)。
board の `defaultThreadAcl`/`defaultPostAcl` はテンプレートで、スレッド/投稿作成時に
`instantiateAcl(template, creatorUserId)` で `ownerUserId` だけを作成者で上書きした ACL を複製する
(旧 `$CREATOR`/`$PARENTS` の文字列テンプレート展開を、単純な構造クローンに置き換えた)。

`roles`/`user_roles` テーブルは板ごとに自由な名前付きロールを作れるよう、そのまま流用する。
`identityService`/`authService` のユーザー・ロール管理権限 (`isUserAdmin` フラグの単純判定) は
この ACL とは独立した別の仕組み。
...
| `boards` | 掲示板 (id, name, acl, default_thread_acl, default_post_acl, ...) |
| `threads` | スレッド (id, board_id, title, acl, post_count, ...) |
| `posts` | 投稿 (id, thread_id, post_number, acl, content, is_deleted, ...) |
| `images` | アップロード画像 (id, storage_key, status, ...) — 旧 imageUploader プラグインから統合 |
...
以前は本体と KV/D1 を共有するだけの独立 Cloudflare Workers として分離されていたが、
コードレベルの結合がゼロ（型定義やアダプタが別々に重複実装されていた）だったため、
モノレポ統合時に整理した。

| 旧プラグイン | 対応 |
|---|---|
| `turnstileApiToken` | `src/features/turnstile/` として本体に統合。`GET/POST /auth/turnstile` |
| `imageUploader` | `src/features/imageUpload/` として本体に統合。`POST /upload/request` 等。DBも本体のD1に統合 (`images` テーブル) |
| `twoCh` (2ch互換dat配信) | 廃止 |
| `datImport` (5ch dat取り込み) | 廃止 |
