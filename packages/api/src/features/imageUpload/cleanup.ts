...
export async function runCleanup(env: AppEnv['Bindings']): Promise<{ deleted: number; errors: number }> {
  ...
  const [expired, abandoned] = await Promise.all([
    repository.findExpiredImages(env.DB!, now),
    repository.findAbandonedPendingImages(env.DB!, abandonedBefore),
  ])
  ...
  for (const image of [...expired, ...abandoned]) {
    try {
      ...
      await repository.deleteImageRow(env.DB!, image.id)
      deleted++
    } catch (e) {
