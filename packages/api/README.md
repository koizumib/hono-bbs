...
> Turnstile の設定 (`TURNSTILE_SITE_KEY`, `TURNSTILE_SECRET_KEY` 等) は本体の `vars`/secret にそのまま設定します（`GET /auth/turnstile` として本体に統合済み）。
...
> Turnstile / 画像アップロード関連の環境変数一覧は [`docs/env-vars.md`](./docs/env-vars.md) を参照してください。
...
- [ ] `ENABLE_TURNSTILE=true` と `TURNSTILE_SECRET_KEY` / `TURNSTILE_SITE_KEY` を設定した (Turnstile を使用する場合)
